...
Usage:
    ...
    uv run python -m sharpa_rl_unilab.tools.benchmark_sharpa_init_dr_construct --env-nums 256,512,1024
    uv run python -m sharpa_rl_unilab.tools.benchmark_sharpa_init_dr_construct --variant-counts 1,2,4,8
    uv run python -m sharpa_rl_unilab.tools.benchmark_sharpa_init_dr_construct --measure construct_plus_pool
"""
...
CONF_DIR = Path(__file__).resolve().parents[1] / "conf" / "ppo"
...
from unilab.utils.device import get_device_info_dict, get_device_info_line
...
DEFAULT_OUTPUT_DIR = Path.cwd() / "logs" / "benchmark" / "sharpa_init_dr_construct"
...
def _init_dr_mode(enabled: bool) -> Iterator[None]:
    from sharpa_rl_unilab.tasks.sharpa_inhand.rotation import SharpaInhandRotationDRProvider
...
def _construct_once(
    ...
    *,
    ...
    adapter = BackendAdapter(cfg, root_dir=Path.cwd(), algo_name="ppo")
