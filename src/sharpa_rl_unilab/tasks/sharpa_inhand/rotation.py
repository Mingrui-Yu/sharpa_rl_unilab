...
from sharpa_rl_unilab.assets import resolve_asset, resolve_scene
...
def resolve_grasp_cache_files(cache_file: str) -> str:
    """Resolve a vendored grasp cache file without any network access.

    Bundled caches ship in the wheel; newly collected caches live under the
    writable asset cache root. Raises ``FileNotFoundError`` when neither holds
    the file.
    """
    return str(resolve_asset(cache_file))
...
def _materialize_grasp_caches(
    ...
    for scale_value in np.asarray(scale_values, dtype=np.float64):
        ...
        try:
            ...
            continue
        grasp_caches.append(np.load(resolved).astype(np.float64))
    ...
    if missing_files:
        ...
        raise RuntimeError(
            ...
            "Generate them with:\n"
            "  uv run sharpa-train --algo ppo --task sharpa_inhand_grasp --sim mujoco "
            "'env.domain_rand.scale_list=[<scale>]' training.no_play=true\n"
            "See docs/en/1-dexterous_inhand.md"
        )
