"""HORA APPO checkpoint playback, owned by this package (UniLab issue #1547).

The HORA APPO *training* runner lives in ``sharpa_rl_unilab.algos.hora``
(extracted from uni_rl); the play-mode orchestration is owner-side business
logic because it drives UniLab's registry-backed env construction, sim2sim
contract validation, and backend playback plan logging.
``resolve_hora_appo_runtime`` is the owner config's ``runtime_resolver``
entrypoint consumed by ``uni_rl.algos.appo.runtime.resolve_appo_runtime``.
"""
...
from sharpa_rl_unilab.algos.hora.observations import (
...
from sharpa_rl_unilab.algos.hora.runtime import is_hora_appo_runtime
