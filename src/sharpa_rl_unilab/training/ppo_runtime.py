...
from .policy import make_models, observe_new_samples, tensor_obs
...
def train_ppo(cfg, progress_key, target, started):
    ...
    with ExitStack() as resources:
        ...
        while counters[progress_key] < target:
            ...
            storage = (
                learner.storage
                if learner is not None and learner.storage.num_transitions_per_env == t
            ...
            with torch.no_grad():
                learner.compute_returns(
                    TensorDict(tensor_obs({"critic": obs["critic"]}, device), batch_size=n)
                )
