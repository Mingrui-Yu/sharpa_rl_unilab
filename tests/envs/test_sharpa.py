...
from sharpa_rl_unilab.tasks.sharpa_inhand.base import (
    SOURCE_DEFAULT_HAND_JOINT_POS_DEG,
    SharpaInhandBaseEnv,
    resolve_grasp_cache_file,
)
from sharpa_rl_unilab.tasks.sharpa_inhand.rotation import (
    SharpaInhandRotationDRProvider,
    _materialize_grasp_caches,
)

_CONF_DIR = Path(__file__).resolve().parents[2] / "src" / "sharpa_rl_unilab" / "conf"
...
def test_sharpa_env_uses_backend_contract_for_mujoco_metadata() -> None:
    ...
    source = "\n".join(
        (_SRC_DIR / "sharpa_rl_unilab" / "tasks" / "sharpa_inhand" / path).read_text(
            encoding="utf-8"
        )
...
def test_sharpa_grasp_cache_is_materialized_once_and_reset_samples_memory(
    ...
    from sharpa_rl_unilab.tasks.sharpa_inhand import rotation
